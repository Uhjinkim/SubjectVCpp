                     ...
                     _In_ int       nCmdShow)
{
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MTE));
    ...
    case WM_PAINT:
        {
            ...
            //버튼 크기. 위치 조정
            btn.width = (layout.right - layout.left) / 5 - btn.margin*2;
            for (int i = 0; i < BUTTON_COUNT; i++) {
                SetWindowPos(hbuttons[i], NULL, layout.left + btn.margin + ((btn.margin + btn.width) * i), layout.top + btn.margin, btn.width, btn.height, SWP_NOZORDER);
            }
            //컨테이너 박스, 드로잉영역 그리기
            ...
            bool isStartInCanvas = start.x > canvas.left && start.y > canvas.top && start.x < canvas.right && start.y < canvas.bottom;
            bool isEndInCanvas = end.x > canvas.left && end.y > canvas.top && end.x < canvas.right && end.y < canvas.bottom;
